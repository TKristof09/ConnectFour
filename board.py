...
class Board():
    def __init__(self, sizex, sizey, columns=None):
        if columns is None:
            ...
            self.board = columns
            self.sizex = len(columns)
            self.sizey = len(columns[0].get_array())
    ...
    def _check_diag(self, startx, starty, double=False):
        ...
        # diagonal towards the left
            ...
            c = 0
            while r in range(starty, min(startx, self.sizey) + 1) and c in range(startx):
                if arr[c][r] == arr[c + 1][r - 1] and arr[c][r] != 0:
                    count += 1
                else:
                    count = 0
    ...
    def drop(self, column):
        ...
        self.board[column].drop((-1) ** self.current_player)
        self.current_player = 1 - self.current_player
        return self.current_player
    
    # Converts board to 2D array [column][row]
    def get_as_2Darray(self):
        return [self.board[i].get_array() for i in range(len(self.board))]
